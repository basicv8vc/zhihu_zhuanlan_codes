...
class LSTMMHSAN(nn.Module):
    def __init__(self, vocab_sz, i_sz, h_sz=512, h=8, dk=64, is_bidirection=True, dropout=0.3):
        ...
        self.trans_dk = nn.Linear(h_sz, h_sz) # map Qt to same space with Kt
    ...
    def forward(self, x, is_mask=False):
        ...
        Qt = self.trans_q(output) # (bs, seq_len, h*dk)
        Kt = self.trans_k(output).view(-1, seq_len, self.dk) # (bs, seq_len, h, dk)
        Vt = self.trans_v(output).view(-1, seq_len, self.dk) # (bs, seq_len, h, dk)
        Qtt = self.trans_dk(Qt).view(-1, seq_len, self.dk)
        attn = torch.bmm(Qtt, Kt.transpose(1, 2)) # (bs*h, seq_len, seq_len)
        if is_mask:
